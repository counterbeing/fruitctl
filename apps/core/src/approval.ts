...
export class ApprovalEngine {
  ...
  constructor(db: AppDatabase, options?: { registry?: ActionRegistry }) {
    this.db = db;
  ...
  async list(options?: ListOptions): Promise<Proposal[]> {
    const status = options?.status as
      | typeof proposals.$inferSelect.status
      | undefined;
    const rows = status
      ? this.db
          .select()
          ...
          .all()
      : this.db.select().from(proposals).orderBy(proposals.createdAt).all();
