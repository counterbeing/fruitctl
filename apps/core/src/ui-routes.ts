...
function renderCard(p,isPending){var h='<div class="card"><div class="title">'+esc(p.plugin+'.'+p.action)+'</div>';h+='<div class="meta">ID: '+esc(String(p.id))+' | Created: '+esc(p.created_at||'')+'</div>';if(p.params){h+='<div class="params">'+esc(JSON.stringify(p.params))+'</div>'}if(isPending){h+='<div class="actions"><button class="btn-approve" onclick="decide('+p.id+',\\'approve\\')">Approve</button><button class="btn-reject" onclick="decide('+p.id+',\\'reject\\')">Reject</button></div>'}else{var cls='status-'+p.status;h+='<div><span class="status '+esc(cls)+'">'+esc(p.status)+'</span></div>'}h+='</div>';return h}
function fetchProposals(){var t=getToken();if(!t){return}fetch('/proposals',{headers:{Authorization:'Bearer '+t}}).then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}).then(function(data){var items=data.proposals||data||[];var pe='';var hi='';for(var i=0;i<items.length;i++){if(items[i].status==='pending'){pe+=renderCard(items[i],true)}else{hi+=renderCard(items[i],false)}}document.getElementById('pending').innerHTML=pe||'<div class="empty">No pending proposals</div>';document.getElementById('history').innerHTML=hi||'<div class="empty">No resolved proposals</div>';document.getElementById('error').style.display='none'}).catch(function(e){document.getElementById('error').textContent=e.message;document.getElementById('error').style.display='block'})}
